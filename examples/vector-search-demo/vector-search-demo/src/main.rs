...
use diskann::{
    ...
    utils::{file_exists, load_ids_to_delete_from_file, load_metadata_from_file, round_up, Timer},
};
...
const VECTOR_FILE: &str = "large_embeddings.json";
...
fn main() -> ANNResult<()> {
    ...
    let insert_points: Vec<Vec<f32>> = items
        .into_iter()
        .take(100000) // Test with first 10K vectors to monitor disk usage
    ...
    */
    ...
    let mut store: MemANNStore<f32> = MemANNStore::new(
        ...
        alpha,
        num_threads,
        1_000_000, // Increased to handle 1M vectors
    )
    .unwrap();
