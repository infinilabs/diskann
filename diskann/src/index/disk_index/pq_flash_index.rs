...
use crate::utils::{div_round_up, round_up};
...
macro_rules! READ_U64 {
    ($file:ident, $val:expr) => {
        ...
        match $file.read_u64() {
            Ok(val) => val,
            Err(e) => return Err(e.into()), // 支持错误类型转换
        }
        */
        *$val = $file.read_u64()?
    };
...
pub struct PQFlashIndex<T, LabelT> {
    reader: std::shared_ptr<AlignedFileReader>,
    _max_node_len: u64,
    ...
    _filter_to_medoid_ids: std::collections::HashMap<LabelT, Vec<u32>>,
    ...
    _label_map: std::collections::HashMap<String, LabelT>,
}
...
// read index metadata
    ...
    let index_metadata = File::open(_disk_index_file, File::Read)?;

    uint32_t nr, nc; // metadata itself is stored as bin format (nr is number of
                     // metadata, nc should be 1)
    index_metadata.read_u32(&nr)?;
    index_metadata.read_u32(&nc)?;
    ...
    let _num_frozen_points;
    READ_U64(index_metadata, _num_frozen_points);
    uint64_t file_frozen_id;
    READ_U64(index_metadata, file_frozen_id);
    if (_num_frozen_points == 1)
        this->_frozen_location = file_frozen_id;
    
    if (_num_frozen_points == 1) {
        // Detected frozen point in index at location _frozen_location Will not output it at search time.
    }
